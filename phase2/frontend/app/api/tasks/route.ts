...
export async function POST(req: Request) {
  try {
    ...
    const { title, description, priority, tags, due_date, dueDate, recurring_interval, recurringInterval } = body;

    // Correct value pick karo
    const finalDueDate = dueDate || due_date || null;
    const finalRecurring = recurringInterval || recurring_interval || null;
    const finalTags = tags || []; // Agar tags na hon to empty array

    const { rows } = await pool.query(
      `INSERT INTO "task"
       ("userId", title, description, priority, tags, "dueDate", "recurringInterval", "createdAt", "updatedAt")

...
const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, onSubmit, task }) => {
  ...
  const [formData, setFormData] = useState<CreateTaskRequest>({
    ...
    recurringInterval: '', // Changed from recurrence_pattern
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (task) {
        ...
        const recurVal = task.recurringInterval || task.recurring_interval || (task as any).recurringInterval || '';
        ...
        setFormData({
          ...
          recurringInterval: recurVal,
        });

        // Set Checkbox state based on whether recurrence interval exists
        setIsRecurring(!!recurVal || task.recurring || false);
  ...
  const handleRecurringToggle = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!checked) {
      // Agar uncheck kiya to pattern clear kardo
      setFormData(prev => ({ ...prev, recurringInterval: '' }));
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (isRecurring && !formData.recurringInterval) {
      newErrors.recurringInterval = 'Recurrence pattern is required';
    }
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    {isRecurring && (
                      <div>
                        ...
                        <select
                          name="recurringInterval" // ✅ Matched state key
                          id="recurringInterval"
                          value={formData.recurringInterval || ''}
                          onChange={handleChange}
                          className={`mt-1 block w-full border ${
                            errors.recurringInterval ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
                          } rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white`}
                        >
